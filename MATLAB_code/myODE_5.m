...
%ODE 5 includes torsional spring exponent, and calculates various inertial
    %properties *before* feeding it into the ode function.

function dQ = myODE_5(t,Q,par,F,alpha,tau0,tau_w)
...
m1 = par.m1; %m1 is the mass of the head-thorax (in cm)
m2 = par.m2; %m2 is the mass of the abdomen (in cm) (petiole + gaster)
I1 = par.I1; %Moment of inertia of the head-thorax (in grams*(cm^2))
I2 = par.I2; %Moment of inertia of the gaster (in grams*(cm^2)) 
S_head = par.S_head; %This is the surface area of the object experiencing 
    %drag. In this case, it is modeled as a sphere (in cm^2).
S_butt = par.S_butt; %This is the surface area of the object experiencing 
    %drag. In this case, it is modeled as a sphere (in cm^2).
tsExp = par.tsExp; %This is the torsional spring constant
...
-1).*Q(3)+Q(4)).^tsExp)+(-1).*c.*((-1).*Q(7)+Q(8))+(-1).*F.*L3.*sin(alpha)...
    -tau_w;
